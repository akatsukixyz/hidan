...
client.once('ready', () =>
	client.user
		.setActivity('@Hidan hi', { type: 'WATCHING' })
...
const projectID = process.env.DIALOGFLOW_PROJECT_ID,
	...
	diag = async (channel, message) => {
		const request = {
			session: sessionPath,
			queryInput: { text: { text: message, languageCode: languageCode } }
		};
		const [response] = await sessionClient.detectIntent(request),
			result = response.queryResult;
		if (!response || !result)
			return await sendMessage(channel, `Sorry, I didn't get that!`);
		if (result.intent.displayName === 'help message') {
			const { author, desc, intents } = JSON.parse(
